...
class GLMAttention(nn.Layer):
    ...
    def _core_parallel_attention(self, hidden_states: Tensor, cache: Tensor = None):
        ...
        if cache is None:
            ...
            mixed_q_layer = mixed_q_layer[:, :, -query_length:]
        if DEBUG:
            print("block[cache k]: ", mixed_k_layer[0,0,0,:].cpu().numpy().reshape(-1).tolist()[:20])
            print("block[cache v]: ", mixed_v_layer[0,0,0,:].cpu().numpy().reshape(-1).tolist()[:20])
...
class GLMStack(nn.Layer):
    ...
    def __init__(self, config: GLMConfig):
        ...
        print("!!!!! create transformer_block")
        ...
        print("!!!!! end create transformer_block")
...
class GLMPretrainedModel(PretrainedModel):
    ...
    def set_state_dict(self, state_dict, use_structured_name=True):
        ...
        num_attention_heads = self.config.num_attention_heads // self.config.tensor_parallel_degree
